...
public class PriorityQueueInefficient<T extends Comparable<? super T>> implements SimpleQueue<T> {
	//It is inefficient because for my dequeue method I am using the swap method for each comparison rather than swapping when it is necessary
	//Exactly 2 private fields:
	//A reference to an ArrayList<T>. This is where you will store your binary heap. 
	//A boolean to keep track of whether the queue is a minimum queue (where the minimum value is at the top of the heap) or a maximum queue (where the maximum value is at the top of the heap). 
	...
	public T dequeue() {
		...
		//remove and save the root element
		int originalSize = size(); //also equal to the index of last element of the current list before dequeueing
		// System.out.println("originalSize is " + originalSize);
		T rootElement = queue.remove(1); //1 is the root
			...
			switch (priority) {
				...
				//has to have at least 1 child if it gets through the first size if statement
					...
					while (childIndex < originalSize) {
						if (queue.get(holeIndex).compareTo(queue.get(childIndex)) > 0) { //if the hole index is greater than its (lesser) child index
							Collections.swap(queue,holeIndex,childIndex); //swap
							...
							return rootElement; //out of while loop
						}
					}			
					return rootElement;
					// break;
					...
					while (childIndex < originalSize) {
						if (queue.get(holeIndex).compareTo(queue.get(childIndex)) < 0) { //if the hole index is greater than its (lesser) child index
							...
							if (holeIndex*2 >= originalSize) {
								...
								childIndex = holeIndex*2; //set childIndex equal to the left child
							} else { //if hole has 2 children
								//find the greater child index
								if (queue.get(holeIndex*2).compareTo(queue.get((holeIndex*2) + 1)) >= 0) {
									childIndex = holeIndex*2;
	...
	public void enqueue(T element) {
		...
		switch (priority) {
			...
			case "min":
				...
				// else {
				// 	break;
				// } //leave the added element in the queue if nothing has been enqueued yet.
