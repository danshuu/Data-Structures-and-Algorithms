...
public class MazeGenerator
{
    ...
    private void init_cells()
    {
        ...
        for (i = 0; i < ROWS; i++)
            for (j = 0; j < COLS; j++)
                maze[i][j] = (N + E + S + W); // why set every cell equal to 15? *******

        // create stack for storing previously visited locations
        Stack = new int[ROWS*COLS][2]; // why is this 2? ******

        // initialize stack
        for (i = 0; i < ROWS*COLS; i++) // whats the point of the stack? *******
            for (j = 0; j < 2; j++)
                Stack[i][j] = 0;
    }
    ...
    {
        int i, j, r, c;
        int[] curr = new int[2]; // 2 because it will contain a row and a column
        Random rand = new Random(); //declare a random variable
        ...
        int[][] move = {{-1, 0}, { 0, 1}, { 1, 0}, { 0,-1}}; //how does this work?***
        int[][] next = {{0, 0}, { 0, 0}, { 0, 0}, { 0,0}}; // why? *******

        // choose a cell at random and make it the current cell
        r = rand.nextInt(ROWS); // pick a random cell for what/who?
        c = rand.nextInt(COLS);

        // current search location
        curr[0] = r; //set currentl location to random r and c
        curr[1] = c;
        int visited  = 1;
        int total = ROWS*COLS;
        int tos   = 0;                              // index for top of cell stack

        while (visited < total) // while visited cells is less than total amount of cells?
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                if ((0 <= r) && (r < ROWS) && (0 <= c) && (c < COLS))
                {
                    ...
                    //how does this work???? ****** maze[r][c] & N == N ??
