...
public class Square implements DelimitedTextIO {
	...
	//Constructors
	//Constructor that only takes a row and a column
	...
	public String toText(char delimiter) {
		return getClass().getName() + delimiter + row() + delimiter + col() + delimiter + squareWall[UP] + delimiter +
		squareWall[RIGHT] + delimiter + squareWall[DOWN] + delimiter + squareWall[LEFT] + delimiter + seen() + inView();
	}

	public void toObject(Scanner input) {
		//why no rows and columns?
		squareWall[UP] = input.nextBoolean();
		squareWall[RIGHT] = input.nextBoolean();
		squareWall[DOWN] = input.nextBoolean();
		squareWall[LEFT] = input.nextBoolean();
